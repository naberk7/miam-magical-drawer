...
const SecretSantaApp = () => {
  ...
  // Save data when deadline or phase changes
  useEffect(() => {
    if (participants.length > 0) {
      saveData();
    }
  ...
  // Save data when assignments change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      if (distance < 0) {
        setTimeRemaining(null);
        // Auto-trigger draw when deadline passes (only if in registration phase)
  ...
  const loadData = async () => {
    try {
      const result = await window.storage.get('secret-santa-data');
      if (result && result.value) {
        ...
        // Trigger confetti if already drawn
        if (data.phase === 'drawn') {
          setShowConfetti(true);
          setTimeout(() => setShowConfetti(false), 5000); // Show for 5 seconds
        }
  ...
  const saveData = async () => {
    try {
      ...
      await window.storage.set('secret-santa-data', JSON.stringify(data));
    } catch (err) {
  ...
  const handleSubmit = (e) => {
    ...
    // Add participant - avoid center area where form is
    // Generate position either on left/right sides or top/bottom areas
    ...
    if (avoidCenter) {
      ...
      // Position on top or bottom areas
      x = Math.random() * 100;
  ...
  const saveEdit = (id) => {
    ...
    // Check if email already exists (excluding current participant)
    ...
    if (confirm('Are you sure you want to reset participants and assignments? The deadline will remain unchanged.')) {
      ...
      try {
        const data = {
          participants: [],
          phase: 'registration',
          deadline: deadline, // Keep existing deadline
          assignments: []
        };
        await window.storage.set('secret-santa-data', JSON.stringify(data));
  ...
  const performDraw = () => {
    ...
    // Create assignments - each person gives to the next person in shuffled list
    // This ensures no self-assignment and no mutual pairs
    ...
    while (!validAssignment && attempts < maxAttempts) {
      ...
      // Check for self-assignments (shouldn't happen with this algorithm, but double-check)
    ...
    // Create assignments: person at index i gives to person at index (i+1) % length
